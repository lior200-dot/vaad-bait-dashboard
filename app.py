...
if uploaded_file is not None:
    try:
        ...
        # ========================================================
        # פירוט תשלומים למשפחה - עם מילוי חודשים ריקים
        # ========================================================
        st.markdown("---")
        ...
        if len(paying_families) > 0:
            ...
            # 2. יצירת ציר זמן מלא (כל חודש בטווח)
            full_date_range = pd.date_range(start=start_date, end=end_date, freq='MS') # MS = Month Start
            
            combined_rows = []
            
            # 3. מיזוג: עבור כל חודש בטווח, בדוק אם יש תשלום
            for date_point in full_date_range:
                month_str = date_point.strftime('%m/%Y')
                
                # מציאת תשלומים בחודש זה (לפי המחרוזת MM/YYYY)
                payments_in_month = actual_payments[actual_payments['Month'] == month_str]
                
                if not payments_in_month.empty:
                    # אם יש תשלומים, הוסף אותם (שומר על הפרדה אם היו כמה תשלומים)
                    for _, row in payments_in_month.iterrows():
                        combined_rows.append(row)
                else:
                    # אם אין תשלום, צור שורת דמה עם סכום 0
                    empty_row = {
                        'Date': date_point,
                        'Credit': 0,
                        'Month': month_str,
            ...
            # עיצוב הטקסט (הסתרת ה-0 כדי שלא יפריע בעין)
