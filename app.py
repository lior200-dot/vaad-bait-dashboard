...
def load_data(uploaded_file):
    try:
        ...
        if len(cols) < 9:
            st.error("קובץ הבנק לא תואם למבנה המצופה (חסרות עמודות).")
            return pd.DataFrame()
        ...
        df['OriginalIndex'] = 'bank_' + df.index.astype(str) 
        
        return df
    except Exception as e:
        st.error(f"שגיאה בטעינת נתוני הבנק: {e}")
        return pd.DataFrame()

# --- פונקציית טעינת נתונים ידניים (מזומן/צ'קים) ---
def load_manual_data(uploaded_file):
    try:
        # קריאת הקובץ (הנחה שהשורה הראשונה היא כותרות)
        df = pd.read_excel(uploaded_file)
        cols = df.columns.tolist()
        
        if len(cols) < 4:
            st.error("קובץ התשלומים הידניים לא תואם. נדרשות 4 עמודות: תאריך, פעולה, פרטים (שם), סכום.")
            return pd.DataFrame()

        # מיפוי עמודות לפי אינדקס: 0-תאריך, 1-פעולה, 2-מי שילם, 3-סכום
        mapping = {
            cols[0]: 'Date',
            cols[1]: 'Action',
            cols[2]: 'Beneficiary',
        ...
        df['Credit'] = pd.to_numeric(df['Credit'], errors='coerce').fillna(0)
        df['Debit'] = 0.0
        df['Balance'] = None # לתשלומים ידניים אין יתרה להצגה בגרף היתרה של הבנק
        
        df['Details'] = "תשלום ידני (" + df['Action'].astype(str) + ")"
        df['Beneficiary'] = df['Beneficiary'].fillna('').astype(str)
        df['Action'] = df['Action'].fillna('').astype(str)
        
        df['Month'] = df['Date'].dt.strftime('%m/%Y')
        df['OriginalIndex'] = 'manual_' + df.index.astype(str)
        
        return df
    except Exception as e:
        st.error(f"שגיאה בטעינת הקובץ הידני: {e}")
        return pd.DataFrame()
...
def display_family_history(family_name, df_full, start_date, end_date, hide_missing_months):
    ...
    if not gdf.empty:
        ...
        # מסגרת מקווקוות לתשלומים באותו חודש
...
uploaded_bank = st.sidebar.file_uploader("1️⃣ בחר קובץ אקסל מהבנק", type=['xlsx', 'xls'])
uploaded_manual = st.sidebar.file_uploader("2️⃣ בחר קובץ תשלומים ידניים (אופציונלי)", type=['xlsx', 'xls'])

df_bank = pd.DataFrame()
df_manual = pd.DataFrame()

if uploaded_bank is not None:
    df_bank = load_data(uploaded_bank)

if uploaded_manual is not None:
    df_manual = load_manual_data(uploaded_manual)

if not df_bank.empty or not df_manual.empty:
    
    # איחוד הקבצים (אם קיימים)
    df_list = [d for d in [df_bank, df_manual] if not d.empty]
    ...
    with c_r1_2:
        st.subheader("מגמת יתרה מצטברת (בנק)")
        # סינון רק של שורות עם יתרה תקינה (מתעלם מהתשלומים הידניים שאין להם יתרה)
        balance_data = df_filtered[df_filtered['Balance'].notna() & (df_filtered['Balance'] > 0)].sort_values('Date')
        if not balance_data.empty:
            fig = px.line(balance_data, x='Date', y='Balance', color_discrete_sequence=['purple'])
            fig.update_layout(yaxis=dict(tickformat=",.0f")); st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("אין נתוני יתרה מהבנק להצגה בטווח זה.")
